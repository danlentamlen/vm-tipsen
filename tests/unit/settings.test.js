...
// ärMatchLåst/byggLåsMap tar `nu` som injicerbar parameter → deterministiska
// tester oavsett när de körs (tidigare gick de sönder när verkliga klockan
// passerade fixturernas matchdatum).
...
describe('Round of 32 — öppnar när gruppspelet låsts (oberoende av resultat)', () => {
  it('öppen om nu är efter GRUPPSPEL_DEADLINE och ingen lock-deadline ännu', () => {
    ...
    // nu = 2026-06-28 → r32 avspark 2026-06-30 → lock 2026-06-29T22:00Z → öppen
    const locked = ärMatchLåst(r32Match, allaMatcher, [], nu)
...
describe('Round of 16', () => {
  ...
  it('låst om inga R32-resultat finns', () => {
    ...
    const locked = ärMatchLåst(r16Match, allaMatcher, tomtResultat, nu)
    expect(locked).toBe(true)
  })

  it('öppen så snart minst ett R32-resultat finns (och utanför lock-fönstret)', () => {
    const locked = ärMatchLåst(r16Match, allaMatcher, resultatMedR32, nu)
    expect(locked).toBe(false)
  })
  ...
  it('locked om vi passerat 4h-deadline (oberoende av R32-resultat)', () => {
    ...
    const locked = ärMatchLåst(passeradMatch, gammaMatcher, resultatMedR32, nu)
    expect(locked).toBe(true)
  })
})
...
describe('Quarter-final', () => {
  ...
  it('låst om inga R16-resultat finns (även om R32 är klar)', () => {
    const locked = ärMatchLåst(qfMatch, allaMatcher, [['r32_01', '2', '1']], nu)
    expect(locked).toBe(true)
  })

  it('öppen när första R16-resultat finns', () => {
    const locked = ärMatchLåst(qfMatch, allaMatcher, resultatMedR16, nu)
    expect(locked).toBe(false)
  })
})
...
describe('Semi-final', () => {
  it('låst utan QF-resultat', () => {
    expect(ärMatchLåst(sfMatch, allaMatcher, [['r16_01', '1', '0']], nu)).toBe(true)
  })

  it('öppen med QF-resultat', () => {
    expect(ärMatchLåst(sfMatch, allaMatcher, [['qf_01', '2', '1']], nu)).toBe(false)
  })
})
...
describe('Final och Match for third place', () => {
  ...
  it('Final: låst utan SF-resultat', () => {
    expect(ärMatchLåst(finalMatch, allaMatcher, [], nu)).toBe(true)
  })

  it('Final: öppen med SF-resultat', () => {
    expect(ärMatchLåst(finalMatch, allaMatcher, sfResultat, nu)).toBe(false)
  })

  it('3rd place: öppen med SF-resultat', () => {
    expect(ärMatchLåst(treMatch, allaMatcher, sfResultat, nu)).toBe(false)
  })
})
...
describe('byggLåsMap', () => {
  it('ger rätt locked-state för hela matchlistan', () => {
    ...
    const map = byggLåsMap(allaMatcher, resultat, nu)
